...
# echo "PEMA for 16S in ENA format and using `fastp` for preprocessing, `vsearch` and Silva 132 db is about to start"
# echo "NCBI Taxonomy IDs will be assigned in the end, on the taxonomies returned."
...
echo "PEMA for 16S in ENA format using using `fastp` for preprocessing, `Swarm` and Silva 132 db is about to start"
echo "NCBI Taxonomy IDs will be assigned in the end, on the taxonomies returned."

sed -i 's/outputFolderName=TEST_16S_FASTP_VSEARCH/outputFolderName=TEST_16S_FASTP_SWARM/' 16S/parameters.tsv
