...
biotic <- read.csv("16S_otu_table.txt", sep = "\t", header=TRUE, row.names = 1) 
...
if (silvaVersion == 'silva132') {
  ...
  #(for silva 128)
  taxonomy <- select(biotic, classification)
  colnames <- c("Root", "Domain", "Superkingdom", "Superphylum", "Phylum", "Class", "Order", "Family", "Genus")
  taxonomy <- separate(taxonomy, classification, colnames, sep = ";", remove = TRUE,
                           convert = FALSE, extra = "warn", fill = "warn")
...
ggsave(filename = "barchart.png", scale = 1, dpi = 300, width = 12)
...
HowManyPhyla <- length(unique(unlist(taxonomy[,c("Phylum")])))
#OR
HowManyPhyla <- length(levels(as.factor(taxonomy$Phylum)))
